...
function FactCard({ fact }: { fact: (typeof FACTS)[number] }) {
  return (
    <div className="flex w-28 shrink-0 items-center gap-1.5 rounded-lg border border-white/15 bg-white/10 px-2 py-1 shadow-lg shadow-black/10 backdrop-blur-md sm:w-96 sm:gap-3 sm:rounded-xl sm:px-5 sm:py-4">
      <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-white/15 text-primary-light sm:h-9 sm:w-9">
        <span className="h-2.5 w-2.5 sm:h-5 sm:w-5">{fact.icon}</span>
      </span>
      <p className="text-[8px] font-medium leading-none text-white/90 sm:text-sm sm:leading-normal">{fact.text}</p>
    </div>
  );
}
